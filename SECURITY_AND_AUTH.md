...
The Railway MCP server supports per-user OAuth for supported MCP clients. Internal API-key routes may still exist for controlled backend/testing use, but public connector users should authenticate through the Supabase OAuth flow.

For public connector users, the intended path is:

```text
AI client -> OAuth authorization flow -> Supabase Auth -> Railway MCP server -> entitlement lookup -> Supabase RLS
```
...
- The reviewing client's callback URL is registered.
...
1. Presents client-compatible authorization metadata.
2. Handles authorization-code + PKCE.
3. Delegates user login to Supabase Auth.
4. Issues/refreshes tokens the AI client can store.
